	...
	wp_localize_script(
		...
		array(
			...
			'i18n'         => array(
				...
				'total'    => __( 'Total Price', 'print-option-addon' ),
			),
		)
	);
}
 ...
 * line total (unit × qty) so the front-end can display WooCommerce-accurate values.
 */
function poa_ajax_calculate_price() {
	...
	wp_send_json_success(
		array(
			...
			'total_html'      => wp_kses_post( wc_price( $line_total ) ),
		)
	);
}

/**
 * Render the Print Option checkbox and Total Price div before the Add-to-Cart button.
 *
 * The .poa-total-price div is always visible and shows the running total
 * (base price × qty when unchecked; base price + print price × qty when checked).
 * JavaScript updates .poa-total-price-amount via AJAX on every relevant change.
 */
function poa_render_print_option() {
	...
	// Initial total price: base product price × 1 (qty defaults to 1).
	$base_price      = $product ? (float) $product->get_price() : 0.0;
	$initial_total   = wc_price( $base_price );
	?>
	<div class="poa-print-option">
		...
		<p class="poa-total-price">
			<strong><?php esc_html_e( 'Total Price:', 'print-option-addon' ); ?></strong>
			<span class="poa-total-price-amount"><?php echo wp_kses_post( $initial_total ); ?></span>
		</p>
	</div>
	<?php
}
